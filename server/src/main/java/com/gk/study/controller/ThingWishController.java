...
public class ThingWishController {
    ...
    public APIResponse unWish(String id) throws IOException {
        ...
        return new APIResponse(ResponeCode.SUCCESS, "移除成功");
    }
    ...
    public APIResponse updateQuantity(String id, Integer quantity) {
        thingWishService.updateQuantity(id, quantity);
        return new APIResponse(ResponeCode.SUCCESS, "更新成功");
    }

    @Access(level = AccessLevel.LOGIN)
    @RequestMapping(value = "/updateSelected", method = RequestMethod.POST)
    public APIResponse updateSelected(String id, Boolean selected) {
        thingWishService.updateSelected(id, selected);
        return new APIResponse(ResponeCode.SUCCESS, "更新成功");
    }

    @Access(level = AccessLevel.LOGIN)
    @RequestMapping(value = "/updateBatchSelected", method = RequestMethod.POST)
    public APIResponse updateBatchSelected(String userId, Boolean selected) {
        thingWishService.updateBatchSelected(userId, selected);
        return new APIResponse(ResponeCode.SUCCESS, "更新成功");
    }
}
