...
class ThingWishServiceImpl extends ServiceImpl<ThingWishMapper, ThingWish> implements ThingWishService {
    ...
    public void createThingWish(ThingWish thingWish) {
        thingWish.setQuantity(1);
        thingWish.setSelected(false);
    ...
    public void updateQuantity(String id, Integer quantity) {
        ...
        UpdateWrapper<ThingWish> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("id", id).set("quantity", quantity);
    ...
    public void updateSelected(String id, Boolean selected) {
        UpdateWrapper<ThingWish> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("id", id).set("selected", selected);
        mapper.update(null, updateWrapper);
    }

    @Override
    public void updateBatchSelected(String userId, Boolean selected) {
        UpdateWrapper<ThingWish> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("user_id", userId).set("selected", selected);
        mapper.update(null, updateWrapper);
    }
}
