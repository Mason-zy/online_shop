...
onMounted(() => {
  // 确保数据类型正确
  pageData.id = Number(route.query.id) || undefined
  pageData.title = route.query.title
  pageData.cover = BASE_URL + '/api/staticfiles/image/' + route.query.cover
  pageData.price = Number(route.query.price) || 0
  pageData.count = Number(route.query.quantity) || 1
  pageData.amount = pageData.price * pageData.count
