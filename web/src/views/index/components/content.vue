<template>
  <div class="content-wrapper">
    <!-- 广告轮播区域（修复图片完整显示） -->
    <div class="ad-container" v-if="adData.length > 0">
      <a-carousel autoplay>
        <div v-for="item in adData" :key="item.id" class="ad-slide">
          <a :href="item.link" target="_blank">
            <img :src="getAdImageUrl(item.image)" class="ad-image"/>
          </a>
        </div>
      </a-carousel>
    </div>
...
.content-wrapper {
  width: 100%;
  margin: 56px auto 0; /* 顶部margin为56px，以避免与导航栏重叠 */
}
